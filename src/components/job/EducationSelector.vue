<template>
  <div class="relative education-selector">
    <!-- 触发按钮 -->
    <button 
    ...
    <transition name="dropdown">
      <div 
        v-show="isOpen" 
        class="absolute z-10 w-40 mt-1 bg-white border border-gray-200 rounded-md shadow-lg left-0 education-dropdown"
      >
        <div class="flex flex-col">
          <div 
            ...
            :class="isSelected(edu) ? 'text-blue-500 font-medium' : 'text-gray-700'"
          >
            {{ edu.label }}
            <span v-if="isSelected(edu)" class="float-right">
...
// 判断选项是否已选中
const isSelected = (edu: Education) => {
  return tempSelectedValue.value === edu.value;
};
...
// 切换教育背景选择
const toggleEducation = (edu: Education) => {
  tempSelectedValue.value = edu.value;
  emit('update:modelValue', edu.value);
};
...
const selectedLabel = computed(() => {
  ...
  const edu = educationOptions.find(edu => edu.value === props.modelValue);
...
const handleClickOutside = (event: MouseEvent) => {
  ...
  const selectorEl = document.querySelector('.education-selector') as HTMLElement;
