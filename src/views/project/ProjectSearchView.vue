<template>
  <div class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-10">
    <div class="container mx-auto px-4">
      ...
      <h2 class="text-3xl font-bold text-gray-900 mb-10 text-center">项目搜索与对接</h2>
      <!-- 搜索框 -->
      <div class="flex justify-center mb-6">
        <input v-model="keyword" @keyup.enter="onSearch" type="text" placeholder="请输入项目名称/项目编号" class="flex-1 max-w-2xl border border-blue-400 rounded-l-lg px-4 py-3 text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-400 text-base" />
        <button @click="onSearch" class="px-8 py-3 rounded-r-lg bg-blue-600 text-white font-semibold hover:bg-blue-700 transition text-base">查找项目</button>
      </div>
      <!-- 筛选栏（发起方类型+领域） -->
      <div class="bg-white rounded-2xl shadow-lg p-6 mb-8">
        ...
        <div class="space-y-4">
          ...
          <div class="flex items-center flex-wrap overflow-x-auto">
            ...
            <div class="flex space-x-2">
              <button
                ...
                :class="initiatorType === item.value ? activeBtn : inactiveBtn"
                class="mb-2"
              >
                {{ item.label }}
              </button>
          ...
          <div class="flex items-center flex-wrap overflow-x-auto">
            ...
            <div class="flex space-x-2">
              <button v-for="item in fieldOptions" :key="item" @click="selectField(item)" :class="isSelected(item) ? activeBtn : inactiveBtn" class="mb-2">{{ item }}</button>
            </div>
          </div>
        </div>
      </div>
          ...
          <tbody>
            <tr v-for="project in projects" :key="project.projectId" class="border-b hover:bg-blue-50 transition">
              <td class="px-4 py-2 text-center">
                <router-link :to="`/project/detail/${project.projectId}`" class="text-blue-600 hover:text-blue-800 font-medium">{{ project.title }}</router-link>
              </td>
              <td class="px-4 py-2 text-center">
              ...
              <td class="px-4 py-2 text-center">
                ...
                <button
                  v-else
                  disabled
                  class="px-2 py-1 rounded-md bg-gray-400 text-white text-xs shadow cursor-not-allowed"
                >
                  已发送申请
                </button>
              </td>
      ...
      <div class="flex justify-center mt-6">
        <button :disabled="currentPage === 1" @click="changePage(currentPage - 1)" class="px-3 py-1 mx-1 rounded bg-gray-200 disabled:opacity-50">上一页</button>
        <span class="mx-2">第 {{ currentPage }} / {{ Math.ceil(total / pageSize) }} 页</span>
        <button :disabled="currentPage === Math.ceil(total / pageSize)" @click="changePage(currentPage + 1)" class="px-3 py-1 mx-1 rounded bg-gray-200 disabled:opacity-50">下一页</button>
      </div>
      <!-- 申请合作弹窗 -->
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 min-w-[400px] max-w-[90vw]">
          ...
          <div class="space-y-4">
            <div>
              ...
              <textarea v-model="applyMessage" rows="4" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" placeholder="请简要说明申请合作的原因和优势..."></textarea>
            </div>
          </div>
          <div class="flex justify-end space-x-3 mt-6">
            <button @click="showApplyModal = false" class="px-4 py-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-400">取消</button>
            <button @click="submitApply" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">提交申请</button>
          </div>
        </div>
      </div>
    </div>
